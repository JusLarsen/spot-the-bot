...
- **Team avatars**: ~104 pixel-art BBQ-food/veggie icons in `public/avatars/`, listed in the client-safe manifest `lib/avatars.ts` (regenerate both icons + manifest with `npm run gen:avatars`, which calls the PixelLab API — needs `PIXELLAB_API_KEY`). The server assigns a **random** avatar at join (`/api/join`); a team changes it via the validated `/api/avatar` route — the requested name must exist in the manifest (`isValidAvatar`), and the write `update()`s only the `avatar` field so it never clobbers scores. Avatars are cosmetic but still server-authoritative like every other RTDB write. Voting-button sprites (`lib/sprites.ts`) are a separate, unrelated set.
- **Session identity persists in `localStorage`** (`stb_role` = "host"; `stb_team` = `{id, name, sessionId}`) so a reload or dropped phone resumes the same team/host via session restore logic rather than spawning a duplicate. Anything that mints a team ID or changes role must keep these keys in sync.
