...
- **Question bank**: 148 samples across 5 categories (`bbq`, `business`, `disney`, `speech`, `movies`). Most AI samples deliberately show common AI tells (training moments); a minority carry the server-only `sneaky` flag, surfaced in `AnswerResponse` so `Play.tsx` can frame the reveal — "even the pros miss these" on a miss, "you caught it" on a hit. Reveal copy is written for a room of 300 non-technical people: plain language, no writing-craft vocabulary (no "aphorism"/"cadence"/"diction"), instructional for plain AI samples and lightly teasing for `sneaky` ones.
