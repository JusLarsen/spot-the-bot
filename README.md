...
A live, team-based **human-vs-AI** detection game for live events.
...
shuffled bank of 231 text samples — for each one the group decides **human** or **AI**.
The host starts a single shared countdown for the whole room (host picks the length at
start — 5, 10, or 15 min; default 10). When time runs out, the highest net score wins
(right answers +1, wrong -1 — guessing nets nothing); ties break by least total answer
time, so there's one clean trophy winner.
...
It's a **training** game first: every easy AI sample is built around one recognizable AI habit
(10 tells, 10 samples each) so teams learn to spot them. A minority are flagged
...
231 samples across 9 categories, each mixing authentic human quotes with AI imitations:

| Category     | Count | Source flavor                                    |
| ------------ | ----- | ------------------------------------------------ |
| `movies`     | 39    | Star Wars / Avengers / Spaceballs lines          |
| `millennial` | 36    | Millennial comedies (Elf, Mean Girls, Zoolander) |
| `genx`       | 34    | Gen X comedies (Caddyshack, Ghostbusters, …)     |
| `bbq`        | 32    | Traeger influencers (Diva Q, Matt Pittman, …)    |
| `business`   | 22    | Business-book quotes + AI office replies         |
| `disney`     | 21    | Famous + obscure Disney/Pixar lines              |
| `speech`     | 18    | Famous / funny speeches                          |
| `alpha`      | 15    | Gen Alpha family films (Encanto, Paddington 2)   |
| `genz`       | 14    | Gen Z comedies (Spider-Verse, Booksmart, …)      |

The four cohort categories are the comedy expansion — they exist so coverage across
generations is auditable. `category` is organizational only: it is never shown to players
and never affects question order.

The easy tier is exactly **10 AI tells x 10 samples each** — half real chatbot habits
(sycophancy, pre-formatted answers, restating your question, over-politeness), half prose
habits ("not just X, it's Y", lists of three, "In today's world", machine vocabulary,
stacked emphasis, the bolted-on moral). The sample carries the lesson; the reveal is a
one fun sentence that names the tell.
...
| `npm run load:test`     | Load-test the live app with ~100 simulated teams (needs `HOST_TOKEN`) |
